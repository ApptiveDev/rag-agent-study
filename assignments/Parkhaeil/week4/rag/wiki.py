...
class WikiRetrievalChain(RetrievalChain):
    ...
    def split_documents(self, docs, text_splitter):
        ...
        # 파일별 순번 chunk_id 부여 (평가 지표 계산용)
        stem_counter: dict[str, int] = {}
        for doc in split_docs:
            stem = Path(doc.metadata.get("filename", "unknown")).stem
            idx = stem_counter.get(stem, 0)
            doc.metadata["chunk_id"] = f"{stem}_{idx}"
            stem_counter[stem] = idx + 1
